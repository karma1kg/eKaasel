...
<%@ page import="bt.gov.voice.dto.FileAttachmentDTO" %>
<%List<ApplicationDTO> dtoList=(List<ApplicationDTO>) request.getAttribute("ClaimedLists");%>

<%List<FileAttachmentDTO> fileAttachmentDTOs=(List<FileAttachmentDTO>) request.getAttribute("tasklist");%>
<html >
<head>
    <title>eKaaSel</title>
</head>
...
<div class="container" id="targetId">
        <form class="card form-horizontal"  id="approvetask"  method="post" enctype="multipart/form-data">
            ...
            <input type="hidden" name="methodName" value="openApplicationPMO" id="idMethodName">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <div class="bootstrap-admin-box-title">Application Details | Submission Date :
                            ...
                            <span>|</span>
                            <input type="text" readonly  value="<%=dtoList.get(0).getApp_Submission_Date()%>" >
                        </div>
                    </div>
                    <div class="panel-body">
                        ...
                        <div  class="md-form form-group row">
                            <div id="check" style="display: none">
                                ...
                                <div class="col-lg-1"></div>
                                <div class="col-lg-9">
                                    <textarea id="dept_remarks" class="form-control field numeric border-secondary scroll-bar textarea-scroll" readonly ><%=dtoList.get(0).getDept_Remark()%></textarea>
                                    <span ></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group" id="buttonList">
                            ...
                            <div class="col-lg-9">
                                ...
                                <button class="btn btn-info" type="button" id="cmdMinistry" onclick="checkDetails('forward'); " name="yesno" id="NoCheck"> <i class="fa fa-forward"></i> Forward To PMO</button>
                        ...
                        <div class="md-form form-group row" id="ifNo" style="display:none">
                            <div id="pmoRemarksDiv">
                                <label class="col-lg-3 control-label"id="tdEnterRemarks" ><strong>Close Remarks:<span style="color:red;font-weight: bold">*</span></strong></label>
                                <div class="col-lg-1"></div>
                            ...
                            <div  id="trSubmitButton">
                                ...
                                <div class="col-lg-9">
                                    ...
                                    <a type="button" class="btn btn-primary" href="<c:url value='/pmLogin/taskList'/>"> <i class="glyphicon glyphicon-arrow-left"></i> Back </a>                                            <!--<input type="button" class="btn btn-primary" value="Submit" id="cmdMinistrySubmit" onclick="fwdSubmit()"></input>-->
                        ...
                        <div class="md-form form-group row" id="ifYes" style="display:none">
                            <div >
                                ...
                                <div class="col-lg-1"></div>
                                <div class="col-lg-9">
                                    <textarea name="pM_Remark" id="pmoRemarks" style="height: 70px" class="form-control"></textarea>
                            ...
                            <div >
                                ...
                                <div class="col-lg-1"></div>
                                <div class="col-lg-9">
                                    <input type="file" name="filler" value="" class="btn btn-primabry" accept="image/jpeg,image/png,.doc,.docx,.pdf,.xlsx,.xls">
                                </div>
                            </div>
                            <div class="mt-2 col-md-12"></div>
                            <div >
...
<script>
    ...
    if($('#currentStatus').val()=='Pmo_forward_to_Pm'&&(($('#previousStats').val()=='PM_Return_PMO')||$('#previousStats').val()=='Ministry_PMO_Returns')) {
        ...
        if($('#divRemark').val()!= 'null' ||$('#cRemarks').val()!= 'null') {
            d_divRemarks.show();
            d_pmo_remarks.show();
        }
    }

    if($('#currentStatus').val()=='Pmo_forward_to_Pm') {
    ...
    function closed_submit(){
        if(c_call()){
            ...
            var doc_type="Prime Minister Closed";
            var url='${pageContext.request.contextPath}/pmLogin/submitPMOClose?document_type='+doc_type;
    ...
    function forward_submit(){
        if(f_call()){
            ...
            var applicationForm =$("#approvetask");
            var doc_type="Prime Minister to PMO";
            var url='${pageContext.request.contextPath}/pmLogin/submitPMOForward?document_type='+doc_type;
    ...
    $(document).ready(function() {
        ...
        var url='${pageContext.request.contextPath}/pmLogin/fetchDocuments?appNo='+appNo;
        $.ajax({
            url:url,
            type: 'GET',
            success: function (res) {
                ...
                if(res.length>0){
                    for(var i in res){
                        ...
                        "<td><a href='${pageContext.request.contextPath}/pmLogin/donwloadFiles?uuid="+res[i].uuid +"'' target='_blank'> View </a></td>" +
                        "</tr>";
                    }
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                for(var i in attachmentDto){
                    ...
                    "<td><a href='${pageContext.request.contextPath} /pmLogin/donwloadFiles?uuid="+attachmentDto[i].uuid +"'' target='_blank'> View </a></td>" +
                    "</tr>";
                }
                $('#publicAttTblId').find('tbody').append(docBtl);
            }
