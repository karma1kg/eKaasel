        ...
        <div class="form-group">
            <div class="table-responsive">``
                <table class="table table-striped table-bordered table-hover" id=taskList>
                    ...
                    <tbody>
                    <%List<WorkFlowDTO> workFlowDTOList = (List<WorkFlowDTO>) request.getAttribute("myTasklist");%>
            ...
            <a type="button" class="btn btn-primary" href="${pageContext.request.contextPath}/pmoLogin"> <i class="glyphicon glyphicon-arrow-left"></i> Go Back </a>
        </div>
    </div>
</div>
...
<div class="panel panel-info">
    ...
    <div class="panel-body">
        <div class="table-responsive" id="pmo_table">
            <table id="pmo_tabs" class="table table-striped table-bordered table-hover">
                <thead>
                <tr>
                    <th>
                        <b style="color: #0e0e0e;">Sl No</b>
                ...
                <tbody>
                <%List<WorkFlowDTO> workFlowDTO2s = (List<WorkFlowDTO>) request.getAttribute("tasklists1");%>
                <%for (int i =0; i < workFlowDTO2s.size(); i++){%>
                <tr>
                    <td><%=1+i%></td>
                    <td> <a href="#" onclick="task_list1('<%=workFlowDTO2s.get(i).getApplication_number()%>')" id="appNo">
                        <%=workFlowDTO2s.get(i).getApplication_number()%></a>
                    </td>
                    <td><%=workFlowDTO2s.get(i).getCurrent_status()%></td>
                    <td><%=workFlowDTO2s.get(i).getRecord_date()%></td>
                </tr>
                <%}%>
                </tbody>
            </table>
...
<div class="panel panel-info">
    ...
    <div class="panel-body">
        <div class="table-responsive" id="tblMinisterFwd">
            <table id="minister" class="table table-striped table-bordered table-hover">
                ...
                <tr>
                    <td><%=1+i%></td>
                    <td> <a href="#" onclick="task_list2('<%=workFlowDTO1s.get(i).getApplication_number()%>')" id="appNo">
  ...
  <script>
      function task_list(appNo){
          var url='${pageContext.request.contextPath}/pmoLogin/empty/getApplicationDetail?appNo='+appNo;
       $('#openLayout_pmo').load(url);
      }
      function task_list1(appNo){
          var url='${pageContext.request.contextPath}/pmoLogin/empty/getApplicationDetail?appNo='+appNo;
          $('#openLayout_pmo').load(url);
      }
      function task_list2(appNo){
          var url='${pageContext.request.contextPath}/pmoLogin/empty/getApplicationDetail?appNo='+appNo;
          $('#openLayout_pmo').load(url);
          //window.location=url;
      }
